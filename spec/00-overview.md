...
- environ `8 %` de chance sur une fenêtre éligible ;
- uniquement après une semaine calme ou une fin de mission ;
- cooldown de `5 semaines` entre deux souvenirs ;
...
- un 3e fil caché, `la personne à l’écharpe bleue`, suit une suite de villes secrètes différente à chaque partie ; l’étape `4` donne le souvenir marquant de l’écharpe, et l’étape `5` se résout dans la ville de départ ;
...
- les étapes suivantes de `romance` / `amitié` n’arrivent pas “gratuitement” d’une semaine à l’autre : il faut soit revenir plus tard dans la même ville, soit y être resté au moins `4 semaines` ;
- il n’y a pas de souvenirs narratifs quand le joueur est `chez les parents`, sauf le cas spécial `Héritage`.
...
- solde initial : `1000 €`
- coût de vie : `500 €/mois`, fixe
- formule réelle : `currentWeeklyFood(week) = 500 * 12 / 52`, soit environ `115,38 € / semaine`
- victoire : `100 pts`
...
Le `bénéfice net estimé` affiché sur une opportunité tient compte du coût de nourriture sur la durée prévue du sitting.
